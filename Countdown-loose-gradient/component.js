...
export default {
	...
	sockets: {
		state() {
			this.$nextTick(() => {
				if (this.running) this.setcounter_loose()
	...
	methods: {
		...
		setcounter_loose: function() {
			...
			if (this.widget.props.format === "custom") {
				this.counter_loose = applyFormat(this.widget.props.customformat)
			} else if (this.widget.props.format === "humanize") {
				if (now.isAfter(this.target) && !this.widget.props.count_up)
					this.counter_loose = "now"
				else if (now.isAfter(this.target) && this.widget.props.count_up)
					this.counter_loose = moment.duration(-duration).humanize(true)
				else this.counter_loose = duration.humanize(true)
			} else {
				this.counter_loose = applyFormat(this.widget.props.format)
			}

			if (!this.running) return
			setTimeout(this.setcounter_loose, 100)
		}  
	}
}
